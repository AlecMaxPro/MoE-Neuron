...
class OlmoeMLP(nn.Module):
    ...
    def forward(self, x):
        na_coeffs = self.act_fn(self.gate_proj(x)) * self.up_proj(x)
        return self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x)), na_coeffs
...
class OlmoeAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        b_attn_output = torch.matmul(attn_weights, value_states)

        if b_attn_output.size() != (bsz, self.num_heads, q_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_heads, q_len, self.head_dim)}, but is"
                f" {attn_output.size()}"
            )

        attn_output = b_attn_output.transpose(1, 2).contiguous()
        ...
        return attn_output, attn_weights, attn_subvalues, b_attn_output
...
class OlmoeSdpaAttention(OlmoeAttention):
    ...
    def forward(
        ...
        attn_weights, b_attn_output = _scaled_dot_product_attention(
            query_states,
            key_states,
            value_states,
...
class OlmoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        experts_na_coeffs = torch.zeros(
            (batch_size * sequence_length, self.top_k, e_dim),
        ...
        for expert_idx in range(self.num_experts):
            ...
            current_hidden_states, na_coeffs = expert_layer(current_state) 
            current_hidden_states = current_hidden_states * routing_weights[top_x, idx, None]
            
            experts_na_coeffs[top_x, idx] = na_coeffs
            # However `index_add_` only support torch tensors for indexing so we'll use
            # the `top_x` tensor here.
            final_hidden_states.index_add_(0, top_x, current_hidden_states.to(hidden_states.dtype))
        final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, hidden_dim)
        return final_hidden_states, router_logits, experts_na_coeffs, selected_experts, routing_weights
...
class OlmoeDecoderLayer(nn.Module):
    def __init__(self, config: OlmoeConfig, layer_idx: int):
        ...
        self.self_attn = OLMOE_ATTENTION_CLASSES['eager'](config=config, layer_idx=layer_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        na_layer_input = hidden_states
        hidden_states = self.input_layernorm(hidden_states)

        # Self Attention
        hidden_states, self_attn_weights, na_attn_subvalues, b_attn_output = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
        ...
        na_attn_output = hidden_states
        hidden_states = residual + hidden_states
        na_residual_output = hidden_states

        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        
        hidden_states, router_logits, experts_na_coeffs, selected_experts, routing_weights = self.mlp(hidden_states)
        na_ffn_output = hidden_states
        
        hidden_states = residual + hidden_states
        na_layer_output = hidden_states
        ...
        if use_cache:
            outputs += ((na_layer_input, na_attn_output, 
                         na_residual_output, na_ffn_output, 
                         na_layer_output, na_attn_subvalues, 
                         experts_na_coeffs, self_attn_weights, 
                         selected_experts, routing_weights, router_logits, b_attn_output),)
