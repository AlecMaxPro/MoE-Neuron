...
class OlmoSdpaAttention(OlmoAttention):
    ...
    def forward(
        ...
        def _scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
            ...
            # 确保 attn_bias 在正确的设备上
            attn_bias = torch.zeros(L, S, dtype=query.dtype, device=query.device)
            if is_causal:
                assert attn_mask is None
                # 确保 temp_mask 在正确的设备上
                temp_mask = torch.ones(L, S, dtype=torch.bool, device=query.device).tril(diagonal=0)
                attn_bias.masked_fill_(temp_mask.logical_not(), float("-inf"))
                attn_bias.to(query.dtype)

            if attn_mask is not None:
                # 确保 attn_mask 在正确的设备上
        ...
        attn_weights, attn_output = _scaled_dot_product_attention(
            query_states,
            key_states,
            value_states,
...
class OlmoDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, na_coeffs = self.mlp(hidden_states)
        ...
        if use_cache:
            outputs += ((na_layer_input, na_attn_output, na_residual_output, 
                         na_ffn_output, na_layer_output, na_attn_subvalues, 
                         na_coeffs, self_attn_weights, b_attn_output),)
        # print("成功修改")
        return outputs
